...
MP4::Tag::Tag(TagLib::File *file, MP4::Atoms *atoms)
{
    ...
    for(unsigned int i = 0; i < ilst->children.size(); i++) {
        ...
        else if(atom->name == "rati") {
            parseUShort(atom, file);
        }
        else if(atom->name == "purl") {
            parseText(atom, file, -1);
        }
        else if(atom->name == "egid") {
            parseText(atom, file, -1);
        }
        else {
            parseText(atom, file);
...
MP4::Tag::parseData2(MP4::Atom *atom, TagLib::File *file, int expectedFlags, bool freeForm)
{
    ...
    while(pos < data.size()) {
        ...
        //qDebug()<<QByteArray(atom->name.data(),atom->name.size())<<flags;
        if(freeForm && i < 2) {
            if(i == 0 && name != "mean") {
...
void
MP4::Tag::parseUShort(MP4::Atom *atom, TagLib::File *file)
{
  ByteVectorList data = parseData(atom, file);
  if(data.size()) {
    addItem(atom->name, data[0].toUShort());
  }
}
...
MP4::Tag::padIlst(const ByteVector &data, int length)
{
    ...
    return renderAtom("free", ByteVector(length, '\0'));
}
...
ByteVector
MP4::Tag::renderUShort(const ByteVector &name, MP4::Item &item)
{
    ByteVectorList data;
    data.append(ByteVector::fromShort(item.toUShort()));
    return renderData(name, item.atomDataType(), data);
}
    ...
    for(MP4::ItemListMap::Iterator i = d->items.begin(); i != d->items.end(); i++) {
        ...
        if(name.startsWith("----")) {
            ...
            data.append(renderByte(name.data(String::Latin1), i->second));
              ...
              data.append(renderUShort(name.data(String::Latin1), i->second));
            }
        else if(name == "covr") {
